...
parser.add_argument('--model-path',
                    default='./cifar10_lbgat0_wideresnet34-10.pt',
                    help='model for white-box attack evaluation')
parser.add_argument('--source-model-path',
                    default='./model-cifar10-wideResNet_abs_1_2_no_detach/cifar10_lbgat0_0.031_0kl_abs_1_2_no_detach-epoch93.pt',
...
# trainset = TinyImageNet('./tiny-imagenet-200', 'train', transform=transform_test, in_memory=True)
# train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.test_batch_size, shuffle=True, **kwargs)
#
# testset = TinyImageNet('./tiny-imagenet-200', 'val', transform=transform_test, in_memory=True)
# test_loader = torch.utils.data.DataLoader(testset, batch_size=args.test_batch_size, shuffle=False, **kwargs)




def evaluate__standard(test_loader, model):
    test_loss = 0
    test_acc = 0
    n = 0
    model.eval()
...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts):
    ...
    for zz in range(restarts):
        ...
        for _ in range(attack_iters):
            ...
            # print(f'Outputs size: {output.size()}')
...
def evaluate__pgd(test_loader, model, attack_iters, restarts, epsilon=(8 / 255.) / std):
    print(epsilon)
    alpha = (2 / 255.) / std
    pgd_loss = 0
    pgd_acc = 0
...
def main():
    ...
    if args.white_box_attack:
        ...
        # model = nets.ResNet18_cifar(num_classes=10)
        ...
        model.eval()
        AT_models_test_loss, AT_models_test_acc = evaluate__standard(test_loader, model)
        ...
        AT_pgd_loss_10, AT_pgd_acc_10 = evaluate__pgd(test_loader, model, 10, 1, epsilon=(8 / 255.) / std)
        print('AT_models_test_acc:', AT_pgd_acc_10)
        AT_pgd_loss_20, AT_pgd_acc_20 = evaluate__pgd(test_loader, model, 20, 1, epsilon=(8 / 255.) / std)
        print('AT_pgd_acc_20:', AT_pgd_acc_20)
        AT_pgd_loss_50, AT_pgd_acc_50 = evaluate__pgd(test_loader, model, 50, 1, epsilon=(8 / 255.) / std)
